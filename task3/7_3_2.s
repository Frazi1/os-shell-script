.data
    ...
    arg_awk: .string "{split($0, pr, \"\\\"\"); split(pr[1], sec, \"\[\"); split(sec[2], th, \"\:\"); split(pr[3], res, \"\"); if(res[2]==\"4\"){sum[th[1]]+=1}} END { PROCINFO[\"sorted_in\"]=\"@val_num_desc\"; pos = 1; for(i in sum){ if (pos <= 10) {total=total+sum[i]} pos++;};pos = 1; for(i in sum){if (pos <= 10){printf(\"%d. %s - %d - %f%%\\n\",  pos, i, sum[i], sum[i]/total*100);pos++;}}}"
